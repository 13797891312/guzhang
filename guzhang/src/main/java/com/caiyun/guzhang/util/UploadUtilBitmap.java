...
public abstract class UploadUtilBitmap extends AsyncTask<String, Integer, Integer> {
	 ...
	 *            上下文
	 */
	public UploadUtilBitmap(String url, String path, Context context) {
	...
	protected Integer doInBackground(String... params) {
		...
		try {
			path=Bimp.revitionImageSize2(path);
			...
			ContentType contentType = ContentType.create(HTTP.PLAIN_TEXT_TYPE,
					HTTP.UTF_8);
			ContentType contentTypeFile = ContentType.create(
...
//					MainActivity.contextUser.getUnitId(), contentType));
