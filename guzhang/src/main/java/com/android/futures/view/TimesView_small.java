...
public class TimesView_small extends GridChart {
	...
	public TimesView_small(Context context) {
		super(context);
		init();
	}

	public TimesView_small(Context context, AttributeSet attrs) {
		super(context, attrs);
		init();
	}

	public TimesView_small(Context context, AttributeSet attrs, int defStyle) {
	...
	private void drawLines(Canvas canvas) {
		...
		for (int i = 0; i < timesList.size() && i < DATA_MAX_COUNT; i++) {
			...
			if (i != 0) {
				...
				canvas.drawLine(x, uperWhiteY, 3 + start + dataSpacing * i, endWhiteY, paint);//现价


				path.lineTo(3 + start + dataSpacing * i, endWhiteY);

//				paint.setColor(Color.GREEN);
//				canvas.drawLine(x, uperYellowY, 3 + start + dataSpacing * i, endYelloY, paint);//均价
			}
