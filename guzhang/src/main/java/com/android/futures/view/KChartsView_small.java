...
public class KChartsView_small extends GridChart implements
		GridChart.OnTabClickListener {
	...
	/** 默认Y轴字体颜色 **/
	private static final int DEFAULT_AXIS_Y_TITLE_COLOR = Color.RED;
	...
	KDJEntity mKDJData;
	RSIEntity mRSIData6;
	RSIEntity mRSIData12;

	public KChartsView_small(Context context) {
		super(context);
		init();
	}

	public KChartsView_small(Context context, AttributeSet attrs) {
		super(context, attrs);
		init();
	}

	public KChartsView_small(Context context, AttributeSet attrs, int defStyle) {
	...
	private void init() {
		...
		mRSIData6 = new RSIEntity(null, 6);
		mRSIData6 = new RSIEntity(null, 12);
	}
	...
	private void drawCandleDetails(Canvas canvas) {
		if (showDetails) {
			...
			paint.setAlpha(200);
			canvas.drawLine(mStartX, 2.0f + DEFAULT_AXIS_TITLE_SIZE, mStartX,
					UPER_CHART_BOTTOM, paint);
			canvas.drawLine(mStartX, getHeight() - 2.0f, mStartX,
					LOWER_CHART_TOP, paint);
			...
			canvas.drawText("日期: " + mOHLCData.get(selectIndext).getDate(),
					left + 1, top + DEFAULT_AXIS_TITLE_SIZE, textPaint);

			canvas.drawText("开盘:", left + 1, top + DEFAULT_AXIS_TITLE_SIZE
					* 2.0f, textPaint);
			...
			try {
				...
				canvas.drawText(dFormat.format(open), left + 1
						+ DEFAULT_AXIS_TITLE_SIZE * 2.5f, top
						+ DEFAULT_AXIS_TITLE_SIZE * 2.0f, textPaint);
			} catch (Exception e) {
				canvas.drawText(dFormat.format(open), left + 1
						+ DEFAULT_AXIS_TITLE_SIZE * 2.5f, top
						+ DEFAULT_AXIS_TITLE_SIZE * 2.0f, textPaint);
			...
			textPaint.setColor(Color.WHITE);
			canvas.drawText("最高:", left + 1, top + DEFAULT_AXIS_TITLE_SIZE
					* 3.0f, textPaint);
			...
			canvas.drawText(dFormat.format(high), left + 1
					+ DEFAULT_AXIS_TITLE_SIZE * 2.5f, top
					+ DEFAULT_AXIS_TITLE_SIZE * 3.0f, textPaint);

			textPaint.setColor(Color.WHITE);
			canvas.drawText("最低:", left + 1, top + DEFAULT_AXIS_TITLE_SIZE
					* 4.0f, textPaint);
			double low = mOHLCData.get(selectIndext).getLow();
			try {
				double yesterday = (mOHLCData.get(selectIndext + 1).getLow() + mOHLCData
						.get(selectIndext + 1).getHigh()) / 2.0f;
				if (yesterday <= low) {
					textPaint.setColor(Color.RED);
				} else {
					textPaint.setColor(Color.GREEN);
			...
			canvas.drawText(dFormat.format(low), left + 1
					+ DEFAULT_AXIS_TITLE_SIZE * 2.5f, top
					+ DEFAULT_AXIS_TITLE_SIZE * 4.0f, textPaint);

			textPaint.setColor(Color.WHITE);
			canvas.drawText("收盘:", left + 1, top + DEFAULT_AXIS_TITLE_SIZE
					* 5.0f, textPaint);
			double close = mOHLCData.get(selectIndext).getClose();
			try {
				double yesdopen = (mOHLCData.get(selectIndext + 1).getLow() + mOHLCData
						.get(selectIndext + 1).getHigh()) / 2.0f;
				if (yesdopen <= close) {
					textPaint.setColor(Color.RED);
				} else {
			...
			canvas.drawText(dFormat.format(close), left + 1
					+ DEFAULT_AXIS_TITLE_SIZE * 2.5f, top
					+ DEFAULT_AXIS_TITLE_SIZE * 5.0f, textPaint);

			textPaint.setColor(Color.WHITE);
			canvas.drawText("涨跌幅:", left + 1, top + DEFAULT_AXIS_TITLE_SIZE
					* 6.0f, textPaint);
			try {
				...
				canvas.drawText(new DecimalFormat("#.##%").format(priceRate),
						left + 1 + DEFAULT_AXIS_TITLE_SIZE * 3.5f, top
	...
	private void drawTitles(Canvas canvas) {
		...
		// Y轴Titles
		if (start <= 0) {//标题在Y轴内侧
			String text1 = dFormat.format(mMinPrice);
			canvas.drawText(text1, 2, UPER_CHART_BOTTOM - 1, textPaint);
			String text2 = dFormat.format(mMinPrice + (mMaxPrice - mMinPrice)
					/ 4);
			canvas.drawText(text2, 2, UPER_CHART_BOTTOM - getLatitudeSpacing()
					- 1, textPaint);
			String text3 = dFormat.format(mMinPrice + (mMaxPrice - mMinPrice)
					/ 4 * 2);
			canvas.drawText(text3, 2, UPER_CHART_BOTTOM - getLatitudeSpacing()
					* 2 - 1, textPaint);
			String text4 = dFormat.format(mMinPrice + (mMaxPrice - mMinPrice)
					/ 4 * 3);
			canvas.drawText(text4, 2, UPER_CHART_BOTTOM - getLatitudeSpacing()
					* 3 - 1, textPaint);
			String text5 = dFormat.format(mMaxPrice);
			canvas.drawText(text5, start - 10 - text5.length()
					* DEFAULT_AXIS_TITLE_SIZE / 2.0f,
					DEFAULT_AXIS_TITLE_SIZE * 2, textPaint);
		} else {//标题在Y轴外侧
			String text1 = dFormat.format(mMinPrice);
			canvas.drawText(text1, start - 10 - text1.length()
					* DEFAULT_AXIS_TITLE_SIZE / 2.0f, UPER_CHART_BOTTOM - 1,
					textPaint);
			String text2 = dFormat.format(mMinPrice + (mMaxPrice - mMinPrice)
					/ 4);
			canvas.drawText(text2, start - 10 - text2.length()
					* DEFAULT_AXIS_TITLE_SIZE / 2.0f, UPER_CHART_BOTTOM
					- getLatitudeSpacing() - 1, textPaint);
			String text3 = dFormat.format(mMinPrice + (mMaxPrice - mMinPrice)
					/ 4 * 2);
			canvas.drawText(text3, start - 10 - text3.length()
					* DEFAULT_AXIS_TITLE_SIZE / 2.0f, UPER_CHART_BOTTOM
					- getLatitudeSpacing() * 2 - 1, textPaint);
			String text4 = dFormat.format(mMinPrice + (mMaxPrice - mMinPrice)
					/ 4 * 3);
			canvas.drawText(text4, start - 10 - text4.length()
					* DEFAULT_AXIS_TITLE_SIZE / 2.0f, UPER_CHART_BOTTOM
					- getLatitudeSpacing() * 3 - 1, textPaint);
			String text5 = dFormat.format(mMaxPrice);
			canvas.drawText(text5, start - 10 - text5.length()
					* DEFAULT_AXIS_TITLE_SIZE / 2.0f,
					DEFAULT_AXIS_TITLE_SIZE * 2, textPaint);
		...
		canvas.drawText(mOHLCData.get(mDataStartIndext).getDate(), getWidth()
				- 4 - 4.5f * DEFAULT_AXIS_TITLE_SIZE, UPER_CHART_BOTTOM
				+ DEFAULT_AXIS_TITLE_SIZE, textPaint);
		try {
			canvas.drawText(
					String.valueOf(mOHLCData.get(
							mDataStartIndext + mShowDataNum / 2).getDate()),
					getWidth() / 2 - 2.25f * DEFAULT_AXIS_TITLE_SIZE,
					UPER_CHART_BOTTOM + DEFAULT_AXIS_TITLE_SIZE, textPaint);
			canvas.drawText(
					String.valueOf(mOHLCData.get(
							mDataStartIndext + mShowDataNum - 1).getDate()),
					2 + start, UPER_CHART_BOTTOM + DEFAULT_AXIS_TITLE_SIZE,
					textPaint);
		} catch (Exception e) {
	...
	private void drawUpperRegion(Canvas canvas) {
		...
		int width = getWidth();
		mCandleWidth = (width - 4 - start) / 10.0 * 10.0 / mShowDataNum;
		...
		for (int j = 0; j < MALineData.size(); j++) {
			...
			canvas.drawText(
					lineEntity.getTitle()
							+ "="
							+ dFormat.format(lineEntity.getLineData().get(
									mDataStartIndext)), 2 + start
							+ MATitleWidth * j, DEFAULT_AXIS_TITLE_SIZE, paint);
			for (int i = 0; i < mShowDataNum
				...
				if (i != 0) {
					canvas.drawLine(
							...
							(float) ((mMaxPrice - lineEntity.getLineData().get(
									mDataStartIndext + i))
									* rate + DEFAULT_AXIS_TITLE_SIZE + 4),
							paint);
				}
				startX = (float) (width - 2 - mCandleWidth * i - mCandleWidth * 0.5f);
				startY = (float) ((mMaxPrice - lineEntity.getLineData().get(
						mDataStartIndext + i)) * rate);
			}
		}
	}

	private void drawLowerRegion(Canvas canvas) {
		float lowertop = LOWER_CHART_TOP + 1;// 下表顶部
		float lowerHight = getHeight() - lowertop - 4;// 下表高度
		...
		magentaPaint.setAntiAlias(true);
		...
		if (mTabTitle.trim().equalsIgnoreCase("MACD")) {
			...
			String text1 = dFormat.format(high);
			canvas.drawText(text1, start - 10 - text1.length()
					* DEFAULT_AXIS_TITLE_SIZE / 2.0f, lowertop
					+ DEFAULT_AXIS_TITLE_SIZE - 2, textPaint);
			String text2 = dFormat.format((high + low) / 2);
			canvas.drawText(text2, start - 10 - text2.length()
					* DEFAULT_AXIS_TITLE_SIZE / 2.0f, lowertop + lowerHight / 2
					+ DEFAULT_AXIS_TITLE_SIZE, textPaint);
			String text3 = dFormat.format(low);
			canvas.drawText(text3, start - 10 - text3.length()
					* DEFAULT_AXIS_TITLE_SIZE / 2.0f, lowertop + lowerHight,
					textPaint);
			...
			for (int i = mDataStartIndext; i < mDataStartIndext + mShowDataNum; i++) {
				float k = (float) mRSIData6.getRsi().get(i);// 当前rsi
				// Log.e("******",
				// "***"+i+"****"+mOHLCData.get(i).getDate()+"****"+mRSIData.getRsi().get(i));
				float kNext;
				try {
					kNext = (float) mRSIData6.getRsi().get(i - 1);// 下一个rsi
				} catch (Exception e) {
					kNext = -1f;
				}
				float scale = lowerHight / 100;
				if (k != -1f && kNext != -1f) {
					canvas.drawLine(viewWidth - 1 - (float) mCandleWidth
							* (i + 1 - mDataStartIndext) + (float) mCandleWidth
							/ 2, (lowertop + lowerHight) - k * scale, viewWidth
							- 2 - (float) mCandleWidth * (i - mDataStartIndext)
							+ (float) mCandleWidth / 2, (lowertop + lowerHight)
							- kNext * scale, yellowPaint);
				}

				float k12 = (float) mRSIData12.getRsi().get(i);// 当前rsi
				// Log.e("******",
				// "***"+i+"****"+mOHLCData.get(i).getDate()+"****"+mRSIData.getRsi().get(i));
				float k12Next;
				try {
					k12Next = (float) mRSIData12.getRsi().get(i - 1);// 下一个rsi
				} catch (Exception e) {
					k12Next = -1f;
				}
				if (k12 != -1f && k12Next != -1f) {
					canvas.drawLine(viewWidth - 1 - (float) mCandleWidth
							* (i + 1 - mDataStartIndext) + (float) mCandleWidth
							/ 2, (lowertop + lowerHight) - k12 * scale,
							viewWidth - 2 - (float) mCandleWidth
									* (i - mDataStartIndext)
									+ (float) mCandleWidth / 2,
							(lowertop + lowerHight) - k12Next * scale,
							magentaPaint);
				}
			}
			canvas.drawText("100", start - 10 - 3 * DEFAULT_AXIS_TITLE_SIZE
					/ 2.0f, lowertop + DEFAULT_AXIS_TITLE_SIZE - 2, textPaint);
			canvas.drawText("50", start - 10 - 2 * DEFAULT_AXIS_TITLE_SIZE
					/ 2.0f,
					lowertop + lowerHight / 2 + DEFAULT_AXIS_TITLE_SIZE,
					textPaint);
			canvas.drawText("0", start - 10 - 1 * DEFAULT_AXIS_TITLE_SIZE
					/ 2.0f, lowertop + lowerHight, textPaint);
		}
...
//	}
	...
	private void zoomIn() {
		mShowDataNum++;
		if (mShowDataNum > mOHLCData.size()) {
			mShowDataNum = MIN_CANDLE_NUM > mOHLCData.size() ? MIN_CANDLE_NUM
					: mOHLCData.size();
		}
	...
	public void setOHLCData(List<OHLCEntity> OHLCData) {
		...
		mRSIData6 = new RSIEntity(mOHLCData, 6);
		mRSIData12 = new RSIEntity(mOHLCData, 12);
