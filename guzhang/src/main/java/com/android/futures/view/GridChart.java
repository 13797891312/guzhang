...
public class GridChart extends View {
	...
	/** 虚线效果 */
	public PathEffect mDashEffect=DEFAULT_DASH_EFFECT;
	...
	protected void onDraw(Canvas canvas) {
		...
		longitudeSpacing = (viewWidth - 2-start) / (DEFAULT_LOGITUDE_NUM + 1);
	...
	private void drawLongitudes(Canvas canvas, int viewHeight, float longitudeSpacing) {
		...
		for (int i = 1; i <= DEFAULT_LOGITUDE_NUM; i++) {
			Path path=new Path();
			path.moveTo(start + longitudeSpacing * i,topTitleHeight + 2);
			path.lineTo(start + longitudeSpacing * i, UPER_CHART_BOTTOM);
			
			Path path1=new Path();
			path1.moveTo(start + longitudeSpacing * i,LOWER_CHART_TOP);
			path1.lineTo(start + longitudeSpacing * i, viewHeight - 1);
	...
	private void drawLatitudes(Canvas canvas, int viewHeight, int viewWidth, float latitudeSpacing) {
		...
		for (int i = 1; i <= DEFAULT_UPER_LATITUDE_NUM; i++) {
			Path path1=new Path();
			path1.moveTo(start,topTitleHeight + 1 + latitudeSpacing * i);
			path1.lineTo( viewWidth - 1,topTitleHeight + 1 + latitudeSpacing * i);
			canvas.drawPath(path1, paint);
...
//					- latitudeSpacing, paint);
			Path path=new Path();
			path.moveTo(start,viewHeight - 1 - latitudeSpacing);
			path.lineTo( viewWidth - 1,viewHeight - 1
					- latitudeSpacing);
			canvas.drawPath(path, paint);
		}
	...
	private void drawRegions(Canvas canvas, int viewHeight, int viewWidth) {
		...
		paint.setAlpha(150);
		if (showTopTitles) {
			canvas.drawLine(start, 1 + DEFAULT_AXIS_TITLE_SIZE + 2, viewWidth - 1,
					1 + DEFAULT_AXIS_TITLE_SIZE + 2, paint);
		}
		canvas.drawLine(start, UPER_CHART_BOTTOM, viewWidth - 1, UPER_CHART_BOTTOM, paint);
		canvas.drawLine(start, LOWER_CHART_TOP, viewWidth - 1, LOWER_CHART_TOP, paint);
		if (showLowerChartTabs) {
			canvas.drawLine(start, UPER_CHART_BOTTOM + DEFAULT_AXIS_TITLE_SIZE + 2, viewWidth - 1,
					UPER_CHART_BOTTOM + DEFAULT_AXIS_TITLE_SIZE + 2, paint);
			if (mLowerChartTabTitles == null || mLowerChartTabTitles.length <= 0) {
				return;
			}
			mTabWidth = (viewWidth-start - 2) / 10.0f * 10.0f / mLowerChartTabTitles.length;
			...
			for (int i = 0; i < mLowerChartTabTitles.length && mTabWidth * (i + 1) <= viewWidth - 2; i++) {
				if (i == mTabIndex) {
					...
					canvas.drawRect(mTabWidth * i + start, LOWER_CHART_TOP, mTabWidth * (i + 1) + 1+start,
							UPER_CHART_BOTTOM + DEFAULT_AXIS_TITLE_SIZE + 2, bgPaint);
				}
				canvas.drawLine(mTabWidth * i + start, LOWER_CHART_TOP, mTabWidth * i +start,
						UPER_CHART_BOTTOM + DEFAULT_AXIS_TITLE_SIZE + 2, paint);
				canvas.drawText(mLowerChartTabTitles[i], mTabWidth * i +start+ mTabWidth / 2.0f
